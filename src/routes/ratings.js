...
router.post('/',
  ...
  [
    ...
    body('rate').isInt({ min: 1, max: 5 }).withMessage('Rating must be between 1 and 5'),
    body('comment').optional().trim().isLength({ max: 500 }).withMessage('Comment cannot be longer than 500 characters')
  ],
  validateRequest,
  createRating
);
